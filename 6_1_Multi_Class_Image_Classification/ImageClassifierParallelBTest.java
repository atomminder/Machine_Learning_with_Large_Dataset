...
public class ImageClassifierParallelBTest {
	...
	static class SimpleThread extends Thread {
		...
		public void run() {
			...
			String newTrainFile = "specific_training.txt"+inloop;
			//System.out.println(newTrainFile);
	...
	public static void main(String[] args) throws IOException, InterruptedException {
		...
		if(args.length < 2) {
			...
			System.out.println("Usage: ImageClassifierParallelBTest -trainFile -testFile");
			System.exit(1);
		}
       ...
       // while(outloop <= 1) {
		...
		try{
			...
			while ((line = instream.readLine()) != null && line.length() !=0){	
				...
				// get the atrributes
				HashMap<Integer, Double> attributes = normalize(elements[0].split(" "));
				...
				testOutput.append("predictLabel:").append(label).append("\t");
				System.out.println(testOutput.toString());

			}
			instream.close();
			fstream.close();
			System.out.printf("totalTestNumber:%d",totalTest);
		} 
		catch (Exception e){
			e.printStackTrace();
		}	
